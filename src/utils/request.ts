...
class Request {
  // 存放取消请求控制器 Map
  private abortControllerMap: Map<string, ReturnType<typeof Taro.request>>;
  ...
  request<T = any>(config: RequestConfig): Promise<T> {
    ...
    if (enableLoading) {
      Taro.showToast({ title: 'loading', icon: 'loading', mask: true });
    }
    ...
    return new Promise((resolve, reject) => {
      const task = Taro.request({
        ...
        success: (response: Taro.request.SuccessCallbackResult<ApiResult>) => {
          //// hideLoading
          if (enableLoading) {
            Taro.hideToast();
          }
